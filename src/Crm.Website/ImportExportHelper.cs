...
namespace Crm.Website
{
	public class ImportExportHelper
	{
        ...
        public static string GetImportTemplate(FormType formType, Controller controller)
        {
            ...
            List<Crm.Api.FieldInfo> fieldInfos = WebHelper.FormService.GetFields(formType).Where(x => x.CanImport).ToList();
            int i = 0;
            foreach (Crm.Api.FieldInfo filed in fieldInfos)
            {
        ...
        public static string GetUploadImportFileJsonFile(FormType formType, Controller controller)
        {
        ...
        public static List<DataGridColumnModel> GetImportColumns(FormType formType)
        {
            List<Crm.Api.FieldInfo> fields = WebHelper.FormService.GetFields(formType).Where(x => x.CanImport).ToList();
        ...
        public static string Export(List<JObject> models, FormType formType)
        {
            ...
            List<Crm.Api.FieldInfo> fieldInfos = WebHelper.FormService.GetFields(formType).ToList();
